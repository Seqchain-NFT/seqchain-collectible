...
describe("FarmingNFT", function () {
    ...
    const countReward = async function (tokenId: string|number, blocks: number): Promise<BigNumber> {
    ...
    before(async () => {
        ...
        const AlphaGenerationRegistry = await ethers.getContractFactory("AlphaGenerationRegistry");
    ...
    describe('General tests', () => {
        ...
        beforeEach(async () => {
            ...
            farm = await FarmingNFT.deploy(
                ...
                token.address,
                6500,
                [15e18.toString(), 25e18.toString(), 35e18.toString(), 50e18.toString()]
            );
            await farm.deployed()
        })
        ...
        it('#enable fail', async () => {
            await nft.mint(ALICE, 2)
            await expectRevert(farm.connect(ALICE_SIGNER).enable([1, 1]), "Duplicates in token list")
            await expectRevert(farm.connect(BOB_SIGNER).enable([0, 1]), "Ownership not approved")
            await expectRevert(farm.connect(BOB_SIGNER).enable([2]), "ERC721: owner query for nonexistent token")
        })
        ...
        it('#pendingRewardBatch', async () => {
            ...
            await increaseBlockNumber(10)
            assert.notEqual(Number(await farm.pendingRewardBatch([0, 1])), 0, "Empty reward")
        })
        ...
        it('#earn', async () => {
            ...
            console.log(
                acceptedReward.toString(),
                expectedReward.toString()
            )
            assert.equal(String(acceptedReward), String(expectedReward), 'Incorrect reward')
